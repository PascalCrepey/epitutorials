...
covid_count <- covid %>%
  ...
  complete(FECHA_RESULTADO = seq.Date(min(FECHA_RESULTADO, na.rm =T),
                                      ...
                                      by="day"),nesting(DISTRITO), fill = list(n = 0))
...
> Essayez de les présenter sous forme de tableau avec la fonction `kbl()`:
...
`R` dispose d'un vaste univers de packages pour la représentation et l'analyse spatiales. 
...
données spatiales. Dans ce cours, nous nous concentrerons sur la
représentation spatiale de 1) modèles de points et 2) de données
...
> Créez la variable covid_p en utilisant la fonction `st_as_sf()`.
...
###
> Nous allons créer un tracé simple en utilisant la géométrie `ggplot2` et
>`geom_sf()` en ne gardant (`filter()`) que les cas du 11 décembre 2020 (variable `FECHA_RESULTADO`)  .

```{r covid_p_plot, exercise = TRUE}
...
Nous allons maintenant traiter les données de base `covid` et les
...
> nombre d'observations pour chaque date et quartier. Nous construirons
> des données de type « panel », pour lesquelles nous compléterons par «
> 0 » toutes les dates des unités géographiques n'ayant pas déclaré de
> cas.

```{r covid_sf, exercise = T, message = FALSE, warning = FALSE}
covid_count <- covid %>%
  group_by(DISTRITO, FECHA_RESULTADO) %>%
  summarise(casos = n()) %>%
  ungroup() %>%
...
```{r mv_covid_sf, exercise = T, eval=F}
m_sf <- covid_sf %>% 
  filter(FECHA_RESULTADO == "2020-12-11") %>%
...
> Chaque couche est ajoutée sous une géométrie différente avec
> `geom_sf()`. Dans MapView, nous pouvons utiliser le symbole «+» pour
> tracer les deux couches.

```{r twolayers, exercise = T, warning=F, message=F}
ggplot() +
...
> Nous utiliserons la couleur de la géométrie (argument `col`) pour
> représenter le sexe des patients COVID-19 dans la base de données.

```{r sex_layer, exercise = T, warning=F, message=F}
covid_p %>%
  filter(FECHA_RESULTADO == "2020-12-11") %>%
  ggplot() +
...
```{r sex_date, exercise = T, warning=F, message=F}
covid_p %>%
...
```{r sex_date_mv, exercise = T, warning=F, message=F}
m1 <- covid_p %>%
  filter(FECHA_RESULTADO == "2020-04-11") %>%
...
```{r ggspat, exercise = T, warning=F, message=F}
covid_p %>%
  filter(FECHA_RESULTADO == "2020-12-11") %>%
  ggplot() +
  geom_sf(data = covid_sf) +
...
> Nous utiliserons le remplissage de la géométrie (argument « fill »)
...
```{r plot_cases, exercise = T, warning=F, message=F, exercise.setup = "covid_sf"}
covid_sf %>%
  filter(FECHA_RESULTADO == "2020-12-11") %>%
  ggplot() +
...
```{r mv_cases, exercise = T, eval=F, exercise.setup = "covid_sf"}
covid_sf %>% 
  filter(FECHA_RESULTADO == "2020-12-11") %>%
  mapview(layer.name = "Nombre de cas", zcol = "casos")
```
...
```{r facet_plot_cases, exercise = T, warning=F, message=F, exercise.setup = "covid_sf"}
covid_sf %>%
  filter(FECHA_RESULTADO == "2020-04-11" |
           FECHA_RESULTADO == "2020-12-11") %>%
  ggplot() +
...
```{r mv_two_cases, exercise = T, warning=F, message=F, exercise.setup = "covid_sf"}
d1 <- covid_sf %>%
  filter(FECHA_RESULTADO == "2020-04-11") %>%
  mapview(zcol = "casos", layer.name = "2020-04-11 - Nombre de cas")

d2 <- covid_sf %>%
  filter(FECHA_RESULTADO == "2020-12-11") %>%
  mapview(zcol = "casos", layer.name = "2020-12-11 - Nombre de cas")
...
```{r better_visual, exercise = T, warning=F, message=F, exercise.setup = "covid_sf"}
covid_sf %>%
  filter(FECHA_RESULTADO == "2020-12-11") %>%
  ggplot() +
  geom_sf(aes(fill = casos)) +
...
```{r owin, exercise = T, message=F, warning=F}
covid_subset <- covid %>%
...
```{r covid_ppp, exercise = T,message=F,warning=F, exercise.setup = "owin"}
...
```{r density_raster_cov,message=F,warning=F, exercise = T, exercise.setup = "covid_ppp"}
...
```{r mv_density, exercise = T, exercise.setup = "density_raster_cov"}
density_raster_cov |> 
  mapview()
```
...
```{r covid_scan_ppp, exercise = T, message=F, warning=F}
...
> Pour des raisons de coût de calcul, nous utiliserons **49 simulations
...
```{r covid_scan_test, message=F, warning=F, exercise = T, exercise.setup = "covid_scan_ppp"}
...
-   `pvalue` : valeur=p du test d'hypothèse calculé par simulations
    Monte Carlo.

entre autres.

```{r show_covid_scan_test, message=F, warning=F, exercise = T, exercise.setup = "covid_scan_test"}
covid_scan_test
```
...
```{r clust, message=F, warning=F, exercise = T, exercise.setup = "covid_scan_test"}
...
#### Autocorrélation spatiale (globale) : Moran I

Pour effectuer le calcul statistique global de ***Moran I***, nous
...
```{r covid_sf_subset,message=F,warning=F, exercise = T, exercise.setup = "covid_sf"}
...
```{r covid_nb, message=F,warning=F, exercise = T, exercise.setup = "covid_sf_subset"}
...
```{r covid_lw, message=F,warning=F, exercise = T, exercise.setup = "covid_nb"}
